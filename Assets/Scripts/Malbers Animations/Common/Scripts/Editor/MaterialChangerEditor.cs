...
public class MaterialChangerEditor : UnityEditor.Editor
{
    ...
    void HeaderCallbackDelegate(Rect rect)
    {
        ...
        Rect R_2 = new Rect(rect.x + 14 + ((rect.width - 30) / 2), rect.y, rect.width - ((rect.width) / 2),
    ...
    void DrawElementCallback(Rect rect, int index, bool isActive, bool isFocused)
    {
        ...
        if (element.mesh != null)
        {
            ...
            {
                buttonCap = element.mesh.gameObject.activeSelf
                    ? (element.materials[element.current] == null ? "None" : element.materials[element.current].name) +
                      " (" + element.current + ")"
                    : "Is Hidden";
            }
        ...
        EditorGUILayout.BeginVertical(MalbersEditor.StyleGray);
        {
            ...
            if (MyMaterialChanger.showMeshesList)
            {
                if (list.index != -1)
                {
                    ...
                    EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                    EditorGUILayout.PropertyField(Element.FindPropertyRelative("materials"),
                        new GUIContent("Materials"), true);
                    EditorGUILayout.EndVertical();
                    ...
                    {
                        EditorGUILayout.PropertyField(Element.FindPropertyRelative("LODs"),
                            new GUIContent("Meshes", "Has Level of Detail Meshes"), true);
                    }
