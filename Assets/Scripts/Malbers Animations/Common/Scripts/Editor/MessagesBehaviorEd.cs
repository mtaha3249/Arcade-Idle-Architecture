...
public class MessageBehaviorsEd : UnityEditor.Editor
{
    ...
    private void OnEnable()
    {
        ...
        listOnEnter = new ReorderableList(serializedObject, serializedObject.FindProperty("onEnterMessage"), true, true,
            true, true);
        listOnExit = new ReorderableList(serializedObject, serializedObject.FindProperty("onExitMessage"), true, true,
            true, true);
        listOnTime = new ReorderableList(serializedObject, serializedObject.FindProperty("onTimeMessage"), true, true,
            true, true);
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.BeginVertical(MalbersEditor.StyleGray);
        {
            ...
            EditorGUILayout.EndVertical();
            MMessage.UseSendMessage =
                EditorGUILayout.Toggle(new GUIContent("Use Send Message", "Uses the SendMessage() instead"),
                    MMessage.UseSendMessage);
        }
    ...
    void drawElementCallback1(Rect rect, int index, bool isActive, bool isFocused)
    {
        ...
        Rect R_3 = new Rect(rect.x + ((rect.width) / 3) + 5 + 30, rect.y, ((rect.width) / 3) - 5 - 15,
            EditorGUIUtility.singleLineHeight);
        element.typeM =
            (global::TypeMessage) EditorGUI.EnumPopup(R_3,
                element.typeM);
